...
import {UPDATE_EXAM_SETTING, ACEXAMLIST, CREATE_FILTER_DATA_CACHE} from '../_queries';
...
class EditExam<T = {[data: string]: any}> extends React.Component<MarkSubjectExamProps, ExamState> {
  constructor(props: MarkSubjectExamProps) {
    ...
    this.examFilterCacheList = this.examFilterCacheList.bind(
      this
    );
  ...
  async componentDidMount() {
    ...
    console.log('1.test stObj data:', this.state.exObj);
    console.log('30. test ExObj data state:', this.state.ExObj);
    console.log('40. test ExObj data props:', this.props.ExObj);
    this.editInputValue();
  }
  registerSocket() {
  ...
  componentWillReceiveProps() {
    ...
    console.log('check batches:', this.props.batches);
    console.log('2. test stObj data:', this.state.exObj);
    console.log('30. test ExObj data state:', this.state.ExObj);
    console.log('40. test ExObj data props:', this.props.ExObj);
    this.editInputValue();
  }

  async examFilterCacheList() {
  ...
  editInputValue() {
    ...
    console.log('100. test ExObj data:', exValue);
    this.setState({
      examData: {
        ...examData,
        id: exValue.id,
        examName: exValue.examName,
        subject:exValue.subject,
        semester:exValue.semester,
        batches:exValue.batches,
        branch:exValue.branch,
        section:exValue.section,
        exmStDate:exValue.exmStDate,
        exmEdDate:exValue.exmExDate,
        academicYearId: academicYearId,
    ...
    getInput(examData: any) {
      const {branchId, departmentId, academicYearId, exObj} = this.state;
      let inputObj = {
        id:
          examData.id !== null || examData.id !== undefined || examData.id !== ''
            ? examData.id
            : null,
            ...
            exmStDate:examData.exmStDate,
            exmEdDate:examData.exmExDate,
            departmentId: departmentId,
            branchId: branchId,
